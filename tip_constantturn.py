...
class constantturn_EKF:
    ...
    def reset(self, measurement):
        ...
        """
        self.guess_x = np.vstack((np.array([-0.6,  0.35]).reshape(2, 1),self.velocities))
    ...
    def update(self, dt, measurement):
        ...
        self.Prozessmodel_F = np.array([[1, 0, dt, 0],
                           [0, 1, 0, dt],
        ...
        P_pred = self.Prozessmodel_F @ self.Unsicherheit_P @ self.Prozessmodel_F.T + self.Prozessrauschen_Q

        # measured positions (z)
        measurement_z = measurement[:10].reshape(self.num_coordinates, 1)

        # Innovation covariance matrix
        Innovation_S = self.Jacobi_H @ P_pred @ self.Jacobi_H.T + self.Messunsicherheit_R
        
        # Kalman-Gain
        Gain_K = P_pred @ self.Jacobi_H.T @ np.linalg.inv(Innovation_S)  # Kalman gain with numerical stability
        
        # Update state vector and covariance matrix with measurement 
        self.guess_x = x_pred + Gain_K @ (measurement_z - self.Jacobi_H @ x_pred)
        self.Unsicherheit_P = (np.eye(self.vector_shape) - Gain_K @ self.Jacobi_H) @ P_pred
